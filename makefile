...
CLIENT_NCURSES_OBJ=$(OBJ_DIR)/client_ncurses.o
...
CLIENT_NCURSES_BIN=$(BIN_DIR)/client_ncurses
TEST_BIN=$(TEST_BIN_DIR)/unit_tests
INTEGRATION_TEST_BIN=$(TEST_BIN_DIR)/integration_tests

all: $(SERVER_BIN) $(CLIENT_TEXT_BIN) $(CLIENT_NCURSES_BIN)
...
$(CLIENT_NCURSES_BIN): $(CLIENT_NCURSES_OBJ) $(PROTOCOL_OBJ) | dirs
...
$(CLIENT_NCURSES_OBJ): $(SRC_DIR)/client_ncurses.c $(SRC_DIR)/common.h $(SRC_DIR)/protocol.h | dirs
	$(CC) $(CFLAGS) $(CPPFLAGS) -c -o $@ $<
...
ifeq ($(OS),Windows_NT)
	...
	-if exist "$(subst /,\,$(BIN_DIR))" del /Q "$(subst /,\,$(BIN_DIR))\server.exe" "$(subst /,\,$(BIN_DIR))\client_text.exe" "$(subst /,\,$(BIN_DIR))\client_ncurses.exe" 2>nul
	-if exist "$(subst /,\,$(BIN_DIR))" del /Q "$(subst /,\,$(BIN_DIR))\server" "$(subst /,\,$(BIN_DIR))\client_text" "$(subst /,\,$(BIN_DIR))\client_ncurses" 2>nul
	...
	rm -f $(OBJ_DIR)/*.o $(SERVER_BIN) $(CLIENT_TEXT_BIN) $(CLIENT_NCURSES_BIN) $(TEST_BIN) $(INTEGRATION_TEST_BIN)
endif

.PHONY: all test integration-test test-all dirs clean